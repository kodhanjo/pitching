...
def comment(pname):
    ...
    if comments.validate_on_submit():
        comment = Comment(
            comment=comments.comment.data, pitch_id=pitch.id, user_id=current_user.id
        )
...
def update(pname):
    ...
    return render_template(
        "profile/update_pitch.html", pitch=pitches, legend="Update Pitch"
    )
...
def posted(username):
    ...
    page = request.args.get("page", 1, type=int)
    all_pitch = (
        Pitch.query.filter_by(user=user)
        .order_by(Pitch.posted.desc())
        .paginate(page=page, per_page=10)
    )

    return render_template(
        "posted_by.html", pitches=all_pitch, title=user.username, user=user, image=image
    )
