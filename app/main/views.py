...
def index():
    ...
    all_pitch = Pitch.query.order_by(
        Pitch.posted.desc()).paginate(page=page, per_page=10)
...
def upload_pitch():
    ...
    if pitch.validate_on_submit():
        pitch = Pitch(pitch_category=pitch.category.data,
                      pitch=pitch.pitch.data, user=current_user)
...
def comment(pname):
    ...
    image = url_for('static', filename='profile/' +
    ...
    if comments.validate_on_submit():
        comment = Comment(comment=comments.comment.data,
                          pitch_id=pitch.id, user_id=current_user.id)
...
def upvote(id):
    ...
    new_vote = Votes(vote=int(1), user_id=current_user.id,
                     pitches_id=pitch_id.id)
    new_vote.save_vote()
...
def downvote(id):
    ...
    new_vote = Votes(vote=int(2), user_id=current_user.id,
                     pitches_id=pitch_id.id)
    new_vote.save_vote()
    return redirect(url_for('.view_pitch', id=id))
